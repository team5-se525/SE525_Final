...
public class JschExec {
    ...
    protected boolean connectandGetFile(String... args) throws Exception {
        ...
        try {
			...
			if (System.currentTimeMillis() - starttime > interval) throw new Exception("Time elapsed. Thread will close.");
    ...
    private boolean doFileAndSchemeOperation(final ChannelSftp channel,
		...
		if (taskFolder != null) {
			...
			channel.get(fileloc+".scm", GroupProjectMainActivity.getlocalFileDir()+ "/GroupProjectM" + identity + "Task.scm");
			channel.get(fileloc+".scm.sign", GroupProjectMainActivity.getlocalFileDir() + "/GroupProjectM" + identity + "Task.scm.sign");
			channel.get(fileloc+".data", GroupProjectMainActivity.getlocalFileDir() + "/GroupProjectM" + identity + "Task.data");
    ...
    protected void writeTo(String... args) {
        ...
        try {
            ...
            String srcFileLoc = "/GroupProjectM" + targetMachine + "Task";
            String dstFileLoc;
            ...
            channel.put(GroupProjectMainActivity.getlocalFileDir()+srcFileLoc+".scm", dstFileLoc+".scm");
            channel.put(GroupProjectMainActivity.getlocalFileDir()+srcFileLoc+".scm.sign", dstFileLoc+".scm.sign");
            channel.put(GroupProjectMainActivity.getlocalFileDir()+srcFileLoc+".data", dstFileLoc+".data");
...
class PublicKeyTarget {
    ...
    private int port = 22;
    String privateKeyFileName = GroupProjectMainActivity.getlocalFileDir().substring(0, GroupProjectMainActivity.getlocalFileDir().length()-2)+"/se525hw3_rsa";
